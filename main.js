...
fs.readFile(configFilePath, "utf8", (err, data) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line.match(/^ZimraServer:/i)) {
      const colonIndex = line.indexOf(":");
      serverURL = line.substring(colonIndex + 1).trim(); // Extract the server URL from the line
      console.log("Found server URL:", serverURL);
      // Exit the loop once the server URL is found
      break;
    }
  }
        ...
        if (fiscalDayStatus === "FiscalDayClosed") {
          ...
          fs.readFile(configFilePath, "utf8", (err, data) => {
            ...
            if (!VATNumberExists || !previousReceiptDateExists) {
              ...
              check = 0;
              console.log(
                "VATNumber and PreviousReceiptDate already exist. No changes made."
              );
            }
          });
          ...
          fs.readFile(filePath, "utf8", (err, data) => {
            ...
            let updatedData = sections
              .map((section, index) => {
                if (index > 0) {
                  const currenciesExist = section.includes(
                    "CURRENCIES=ZIG|ZiG|ZIG$|ZiG$|zig|Zig|ziG|ZG$|"
                  );
                  ...
                  const exceptExists=section.includes("EXCEPT=ZiG0.00|ZIG0.00|USD0.00|ZWL0.00|0.00|0|EXE|EX|Ex|Non|NON|Exempt")
          ...
          console.log(
            "To perform this upgrade the Fiscal Day has to be closed"
          );
          console.log(
            "Please Generate Z Report to close the current day, then try again."
          );
